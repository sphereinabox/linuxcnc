...
/********************************************************************
* Kinematics for 5 axis 3d printer using ’xyzbc’ mill motion.
* This 3d printer has a tilting table (B axis) and horizontal rotary
* mounted to the table (C axis).
* The extruder is on the A axis, and maps 1:1
********************************************************************/
...
int rtapi_app_main(void) {
    int res = 0;
    comp_id = hal_init("xyzabc-trt-printer-kins");

    if(comp_id < 0) return comp_id;
        haldata = hal_malloc(sizeof(struct haldata));
        if((res = hal_pin_float_new("xyzabc-trt-printer-kins.x-offset",
                  HAL_IO, &(haldata->x_offset),comp_id)) < 0) goto error;
    if((res = hal_pin_float_new("xyzabc-trt-printer-kins.z-offset",
                  HAL_IO, &(haldata->z_offset), comp_id)) < 0) goto error;
    if((res = hal_pin_float_new("xyzabc-trt-printer-kins.tool-offset",
